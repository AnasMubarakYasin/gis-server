...
module.exports = class Logger {
  ...
  #create_stack() {
    const e_stack = new Error().stack
      ...
      .split(" ")
      .reverse().at(0).replaceAll(/[()]/g, '')
    return e_stack.replace(path.join(process.env.PWD) + "/", "");
  }
  http() { }
  /**
   * @param {any} message
   */
  info(message) {
    return this.logger.log("info", message, { stack: this.#create_stack() });
  }
  /**
   * @param {any} message
   */
  warn(message) {
    return this.logger.log("warn", message, { stack: this.#create_stack() });
  }
  /**
   * @param {any} message
   */
  error(message) {
    return this.logger.log("error", message, { stack: this.#create_stack() });
  }
  /**
  ...
  createLogger({ context }) {
    ...
    const logger = this.container.get(context, {
      ...
      format: winston.format.combine(
        ...
        winston.format.printf(
          ({ timestamp, label, level, durationMs, message, tag, stack }) => {
            const durationMsg = durationMs
              ? `${chalk.yellow(durationMs)}ms`
              : "";
            const stack_trace = stack ? chalk.underline(stack) : '.'
            const tagMsg = tag ? `[${tag}]` : "";
            return `[${chalk.magenta(timestamp)}] [${chalk.blue(
              label
            )}] [${level}] ${tagMsg} ${durationMsg} - ${stack_trace} \n>> ${message}`;
          }
        )
      ),
