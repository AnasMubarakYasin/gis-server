...
module.exports = class Activity {
  ...
  constructor(opts) {
    ...
    const timeout = hour_ms + minute_ms;
    const timeout_daily = 24 * 3.6e6;
    create_rotation_daily(timeout, timeout_daily);
    function create_rotation_daily(timeout, timeout_daily) {
      logger.info(`create hourly rotation ${timeout / 3.6e6} hour`);
      const timeout_id = setTimeout(() => {
        logger.info(`timeout hourly rotation`);
        ...
        if (timeout_daily) {
          logger.info(`create daily rotation ${timeout_daily / 3.6e6} hour`);
          create_rotation_daily(timeout_daily);
        }
      }, timeout);
      app.once('close', () => {
    ...
    function create_file_transport() {
      ...
      logger.info(`create file transport ${dest}`);
